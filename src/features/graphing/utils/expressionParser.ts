...
function solveLinearY(left: string, right: string) {
  const leftEval = new Function('x', 'y', `"use strict"; return (${toJavaScriptExpression(left)});`);
  const rightEval = new Function('x', 'y', `"use strict"; return (${toJavaScriptExpression(right)});`);
